...
export default class FreefemRunnerView {
  ...
  scrollDown() {
    let baseElement = this.element
    let divElement =  baseElement.children[0]
    var nbRow = divElement.children.length

    child = divElement.children[nbRow-1]
    child.scrollIntoView({behavior: "auto", block: "end", inline: "nearest"})
  }
}
